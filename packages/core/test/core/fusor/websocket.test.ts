...
const rejectWhenAborted = (signal: AbortSignal): Promise<never> =>
  new Promise((_, reject) => {
    const rejectAborted = () => reject(new Error("handler aborted"));
    if (signal.aborted) {
      rejectAborted();
      return;
    }
...
async function makeFusorWsServer(script: WsServerScript) {
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", (raw) => {
      ...
      if (frame.type === "init") {
        initTimes.push(performance.now());
        inits.push(frame);
...
describe("fusor websocket streaming", () => {
  ...
  it("does not let a detached older save regress the in-memory cursor", async () => {
    const staleSaveGate = Promise.withResolvers<void>();
    ...
    interface CoreCheckpointInternals {
      ...
      rememberProcessed(eventId: string, seq: number): Promise<void>;
    }
    const internals = core as unknown as CoreCheckpointInternals;

    const staleSave = internals.rememberProcessed("evt-old", 1);
    await waitFor(() => staleSaveStarted);
    await internals.rememberProcessed("evt-new", 2);
    staleSaveGate.resolve();
    await staleSave;

    expect(internals.lastProcessedSeq).toBe(2);
  });
  ...
  it("bounds pending event work and replays dropped work from the safe cursor", async () => {
    const tokenSpy = vi.spyOn(cloud, "issueFusorToken").mockResolvedValue({
      token: "t1",
      expiresIn: 900,
    });
      ...
      onInit: (_init, connection) => {
        ...
        if (connection !== 1) {
          return [
            ready,
            eventFrame(
              `evt-backpressure-${FUSOR_WS_MAX_PENDING_EVENTS + 1}`,
              '{"text":"queued"}',
              false,
              FUSOR_WS_MAX_PENDING_EVENTS + 1
            ),
          ];
        }
        return [
    ...
    await waitFor(() => handled === FUSOR_WS_MAX_PENDING_EVENTS + 1);
    expect(server.inits.map((init) => init.startSeq)).toEqual([
      0,
      FUSOR_WS_MAX_PENDING_EVENTS,
    ]);
    expect(handled).toBe(FUSOR_WS_MAX_PENDING_EVENTS + 1);
  });
  ...
  it("enforces the pending byte limit before provider delivery", async () => {
    ...
    const oversized = eventFrame(
      "evt-byte-overflow",
      JSON.stringify({ text: "字".repeat(64) }),
      false,
      1
    );
  ...
  it("releases pending byte accounting after each durable event", async () => {
    ...
    expect(capture.payloads).toEqual([{ text: "first" }, { text: "second" }]);
    expect(server.inits).toHaveLength(1);
  });
  ...
  it("honors a bounded server retry hint", async () => {
    ...
    const server = await makeFusorWsServer({
      ...
      closeAfterInit: (connection) =>
        connection === 1
          ? { code: 1012, reason: "server_draining" }
          : undefined,
    });
    cleanups.push(server.stop);
    ...
    expect(server.initTimes).toHaveLength(2);
    expect(
      (server.initTimes[1] ?? 0) - (server.initTimes[0] ?? 0)
  ...
  it("fails startup instead of retrying a permanent token-mint rejection", async () => {
    const rejection = new SpectrumCloudError(
      401,
      "UNAUTHENTICATED",
    ...
    const core = new FusorCore({
      projectId: "proj",
      projectSecret: "bad-secret",
    });
    core.register(PLATFORM, makeHandler({ payloads: [] }));

    await expect(core.start()).rejects.toBe(rejection);
  ...
  it("treats an abort-aware raw handler rejection as clean user shutdown", async () => {
    const server = await makeFusorWsServer({
      onInit: () => [
        { type: "ready", projectId: "proj", heartbeatIntervalMs: 30_000 },
        eventFrame("evt-abort-close", '{"text":"wait"}', false, 1),
      ],
    });
    cleanups.push(server.stop);
    const handlerStarted = Promise.withResolvers<void>();
    const session = runFusorWsSession({
      onEvent: async (_event, _sendReply, _seq, signal) => {
  ...
  it("preserves a remote close instead of an abort-aware handler error", async () => {
    const server = await makeFusorWsServer({
      onInit: () => [
        { type: "ready", projectId: "proj", heartbeatIntervalMs: 30_000 },
        eventFrame("evt-abort-remote", '{"text":"wait"}', false, 1),
      ],
      closeAfterInit: () => ({
        code: 1012,
        delayMs: 20,
    ...
    await expect(session.done).rejects.toMatchObject({ closeCode: 1012 });
  });
  ...
  it("bounds an uncooperative handler and suppresses late reply and checkpoint", async () => {
    const tokenSpy = vi.spyOn(cloud, "issueFusorToken").mockResolvedValue({
      token: "t1",
      expiresIn: 900,
    });
    ...
    const handler = makeHandler({ payloads: [] });
    handler.messages = async ({ respond, signal }) => {
      handlerStarted = true;
    ...
    try {
      ...
      await core.close();
      expect(performance.now() - startedClosingAt).toBeLessThan(1000);
